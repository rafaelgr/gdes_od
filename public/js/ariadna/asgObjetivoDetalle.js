...
function initForm() {
    ...
    $("#frmAsgObjetivo").submit(function () {
        return false;
    });
...
function asgObjetivoData() {
    ...
    // soporte de combos
    self.posiblesObjetivos = ko.observableArray([]);
    ...
    // valores escogidos
    self.sobjetivoId = ko.observable();
...
function loadData(data) {
    vm.asgObjetivoId(data.asgObjetivoId);
    vm.nombre(data.nombre);
    vm.objetivo(data.objetivo);
    ...
    loadObjetivos(data.objetivo.objetivoId);
...
function loadObjetivos(objetivoId){
    $.ajax({
        type: "GET",
        url: "/api/objetivos",
        dataType: "json",
        contentType: "application/json",
        success: function (data, status){
            vm.posiblesObjetivos(data);
            vm.sobjetivoId(objetivoId);
        },
        error: errorAjax
    });
}
...
function datosOK() {
    $('#frmAsgObjetivo').validate({
        rules: {
            cmbObjetivos: { required: true },
        ...
        messages: {
            cmbObjetivos: {required: 'Seleccione una categoría'},
    ...
    if ($("#txtAsgObjetivo").val() == "") {
        // si no han rellenado el nombre le generamos uno
        vm.nombre($('#cmbObjetivos option:selected').text() + " [" + $('#cmbEjercicios option:selected').text() + "]");
    }

    return $('#frmAsgObjetivo').valid();
}
...
function aceptar() {
    var mf = function () {
        ...
        var data = {
            asgObjetivo: {
                "asgObjetivoId": vm.asgObjetivoId(),
                "nombre": vm.nombre(),
                "objetivo": {
                    "objetivoId": vm.sobjetivoId()
                },
                "ejercicio": {
        ...
        if (asgObjetivoId == 0) {
            $.ajax({
                ...
                success: function (data, status) {
                    ...
                    var url = "AsgObjetivoGeneral.html?AsgObjetivoId=" + data.asgObjetivoId;
