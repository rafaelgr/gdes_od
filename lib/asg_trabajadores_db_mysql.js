...
function comprobarAsgTrabajador(asgTrabajador){
    ...
    comprobado = (comprobado && asgTrabajador.hasOwnProperty("evaluadorF"));
    if (comprobado) {
        comprobado = typeof asgTrabajador.trabajador === "object";
        comprobado = (comprobado && asgTrabajador.trabajador.hasOwnProperty("trabajadorId"));
    }
    comprobado = (comprobado && asgTrabajador.hasOwnProperty("evaluadorI"));
    if (comprobado) {
        comprobado = typeof asgTrabajador.trabajador === "object";
        comprobado = (comprobado && asgTrabajador.trabajador.hasOwnProperty("trabajadorId"));
    }
    comprobado = (comprobado && asgTrabajador.hasOwnProperty("empresa"));
    if (comprobado) {
        comprobado = typeof asgTrabajador.empresa === "object";
        comprobado = (comprobado && asgTrabajador.empresa.hasOwnProperty("empresaId"));
    }
    comprobado = (comprobado && asgTrabajador.hasOwnProperty("fijo"));
    comprobado = (comprobado && asgTrabajador.hasOwnProperty("variable"));
    comprobado = (comprobado && asgTrabajador.hasOwnProperty("variableF"));
    comprobado = (comprobado && asgTrabajador.hasOwnProperty("dFecha"));
    comprobado = (comprobado && asgTrabajador.hasOwnProperty("hFecha"));
	return comprobado;
}
...
function fromDbtoJsAsgTrabajadores2(asgTrabajadores, id) {
    var asgTrabajadoresJs = [];
    for (var i = 0; i < asgTrabajadores.length; i++) {
        asgTrabajadoresJs.push(fromDbtoJsAsgTrabajador2(asgTrabajadores[i], id));
    }
    return asgTrabajadoresJs;
}
...
function fromDbtoJsAsgTrabajador(asgTrabajadorDb){
    var asgTrabajadorJs = {
        ...
        ejercicio: {
            ...
            fechaInicial: asgTrabajadorDb.fechaInicio,
            fechaFinal: asgTrabajadorDb.fechaFinal,
        ...
        evaluadorF: {
            trabajadorId: asgTrabajadorDb.fevaluadorId,
            nombre: asgTrabajadorDb.nevaluadorF
        },
        evaluadorI: {
            trabajadorId: asgTrabajadorDb.ievaluadorId,
            nombre: asgTrabajadorDb.nevaluadorI
        },
        empresa: {
            empresaId: asgTrabajadorDb.empresaId,
            nombre: asgTrabajadorDb.nempresa
        },
        fijo: asgTrabajadorDb.fijo,
        variable: asgTrabajadorDb.variable,
        variableF: asgTrabajadorDb.variableF,
        dFecha: asgTrabajadorDb.dFecha,
        hFecha: asgTrabajadorDb.hFecha,
        responsabilidad: "prueba"
    };
    var resp = "";
    if (asgTrabajadorDb.fevaluadorId) resp += " FUNCIONAL ";
    if (asgTrabajadorDb.ievaluadorId) resp += " JERARQUICA ";
    asgTrabajadorJs.responsabilidad = resp;
    return asgTrabajadorJs;
}

function fromDbtoJsAsgTrabajador2(asgTrabajadorDb, id) {
    var asgTrabajadorJs = {
        asgTrabajadorId: asgTrabajadorDb.asgTrabajadorId,
        nombre: asgTrabajadorDb.nombre,
        trabajador: {
    ...
    var resp = "";
    if (asgTrabajadorDb.fevaluadorId == id) resp += " FUNCIONAL ";
    if (asgTrabajadorDb.ievaluadorId == id) resp += " JERARQUICA ";
    asgTrabajadorJs.responsabilidad = resp;
    return asgTrabajadorJs;
}
...
function fromJstoDbAsgTrabajador(asgTrabajadorJs){
    var asgTrabajadorDb = {
        ...
        variableF: asgTrabajadorJs.variableF,
        dFecha: asgTrabajadorJs.dFecha,
        hFecha: asgTrabajadorJs.hFecha,
        ievaluadorId: asgTrabajadorJs.evaluadorI.trabajadorId,
        fevaluadorId: asgTrabajadorJs.evaluadorF.trabajadorId,
        empresaId: asgTrabajadorJs.empresa.empresaId
    };
...
module.exports.getAsgTrabajadores = function(callback){
    ...
    sql += " asgt.ejercicioId, e.nombre AS nejercicio, e.fechaInicio, e.fechaFinal,";
    sql += " asgt.paisId, p.nombre AS npais,";
    sql += " asgt.unidadId, u.nombre AS nunidad,";
    sql += " asgt.areaId, a.nombre AS narea,";
    sql += " asgt.empresaId, emp.nombre AS nempresa,";
    sql += " asgt.puestoId, pst.nombre AS npuesto,";
    sql += " asgt.fijo, asgt.variable, asgt.variableF,";
    sql += " asgt.dFecha, asgt.hFecha,";
    sql += " asgt.fevaluadorId, t1.nombre AS nevaluadorF,";
    sql += " asgt.ievaluadorId, t2.nombre AS nevaluadorI";
    sql += " FROM asg_trabajadores AS asgt";
    sql += " LEFT JOIN trabajadores AS t ON t.trabajadorId = asgt.trabajadorId";
    sql += " LEFT JOIN trabajadores AS t1 ON t1.trabajadorId = asgt.fevaluadorId";
    sql += " LEFT JOIN trabajadores AS t2 ON t2.trabajadorId = asgt.ievaluadorId";
    ...
    sql += " LEFT JOIN empresas AS emp ON emp.empresaId = asgt.empresaId";
...
module.exports.postAsgTrabajadorEvaluadorBuscar = function (buscador, callback) {
    ...
    sql += " LEFT JOIN puestos AS pst ON pst.puestoId = asgt.puestoId";
    sql += " LEFT JOIN empresas AS emp ON emp.empresaId = asgt.empresaId";
    sql += " WHERE t1.trabajadorId = ? OR t2.trabajadorId = ?";
    sql = mysql.format(sql, [buscador.trabajadorId, buscador.trabajadorId]);
    connection.query(sql, function (err, result) {
        if (err) {
            callback(err, null);
            return;
        }
        asgTrabajadores = fromDbtoJsAsgTrabajadores2(result, buscador.trabajadorId);
        callback(null, asgTrabajadores);
    });
    closeConnectionCallback(connection, callback);
}
